...
const { LERNA_PACKAGE_NAME: PACKAGE_NAME, LERNA_ROOT_PATH } = process.env
if (!PACKAGE_NAME || !LERNA_ROOT_PATH) {
  throw new Error("The build must be run by Lerna; please use `npm run build` from the repository root.")
}

const LERNA_PACKAGE_NAME = PACKAGE_NAME.replace('@minisheep/', '')
...
const EXTERNAL_GLOBALS = SIBLING_PACKAGES.reduce((out, sib) => {
  ...
  three: 'THREE',
  'bidi-js': 'bidi_js',
  '@minisheep/webgl-sdf-generator': 'webgl_sdf_generator',
  'three/examples/jsm/loaders/GLTFLoader.js': 'THREE.GLTFLoader',
  'prop-types': 'PropTypes',
  'object-path': 'objectPath',
  '@minisheep/three-platform-adapter': 'three_platform_adapter'
})
